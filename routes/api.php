...
Route::group(['middleware' => ['api', 'localization']], function () {
    ...
    Route::get('user/find_by_role/{name}', 'App\Http\Controllers\API\UserController@findByRole')->name('user.api.find_by_role');
    Route::get('user/find_by_not_role/{name}', 'App\Http\Controllers\API\UserController@findByNotRole')->name('user.api.find_by_not_role');
